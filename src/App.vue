<template>
  <div class="container">
      ...
      <div class="flex">
        <div class="max-w-xs">
          ...
          <div class="mt-1 relative rounded-md shadow-md">
            <input v-model="ticker" @keydown.enter="add" @input="handleInputChange" type="text" name="wallet" id="wallet"
          ...
          <div v-if="validation" class="text-sm text-red-600">Такой тикер уже добавлен</div>
        </div>
      </div>
      <button @click="add" :disabled="validation" type="button"
    ...
    <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-3">
      <div v-for="t in tickers" :key="t.name" @click='handleChange(t)' :class="{
        'border-4': t === chosen
    ...
    <hr class="w-full border-t border-gray-600 my-4" v-if='tickers.length' />
    <section v-if="chosen" class="relative">
...
export default {
  ...
  methods: {
    ...
    handleInputChange() {
      ...
      this.cryptoData.forEach((name) => {
        if (name.includes(this.ticker.toUpperCase())) {
    ...
    add() {
      ...
      newTicker.interval = setInterval(async () => {
        const f = await fetch(`https://min-api.cryptocompare.com/data/price?fsym=${newTicker.name}&tsyms=USD&api_key=bdd9c98341c80c28459323768736a77bdab43ebdaca5e0b34746c745a473732a`);
        const data = await f.json();
        this.tickers.find((t) => t.name === newTicker.name).price = data.USD > 1 ? data.USD.toFixed(2) : data.USD.toPrecision(2);
        if (this.chosen.name === newTicker.name) {
    ...
    handleDelete(tickerToRemove) {
      ...
      if (tickerToRemove === this.chosen) {
        clearInterval(this.chosen.interval)
        this.chosen = ''
      }
    },
